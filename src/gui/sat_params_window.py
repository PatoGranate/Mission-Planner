...
class SatParamsWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parent = None):
        ...
        # Try finding existing sats, change dropdown preset to inexisting sat
        try: 
            self.parent().sat0
    ...
    def sat_create(self):
        ...
        if sat_name == "":
            sat_name = self.sat_chooser.currentText()
            
        sat = Satellite(a, e, i, omega, raan, anomaly, date, anomaly_type, sat_name)
        
        # Depending on dropdown select, create or update satellite
        if sat_num == 0:
            sat0 = getattr(self.parent(), 'sat0', None)
            if sat0 == None:
                self.parent().sat0 = sat
            else:
                self.parent().sat0.update_orbit(a = a, e = e, i = i, 
                                                omega = omega, raan = raan, 
                                                anomaly = anomaly,
                                                anomaly_type = anomaly_type,
                                                label = sat_name)
                
        elif sat_num == 1:
            sat1 = getattr(self.parent(), 'sat1', None)
            if sat1 == None:
                self.parent().sat1 = sat  
            else:
                self.parent().sat1.update_orbit(a = a, e = e, i = i, 
                                                omega = omega, raan = raan, 
                                                anomaly = anomaly,
                                                anomaly_type = anomaly_type,
                                                label = sat_name)
            ...
            sat2 = getattr(self.parent(), 'sat2', None)
            if sat2 == None:
                self.parent().sat2 = sat  
            else:
                self.parent().sat2.update_orbit(a = a, e = e, i = i, 
                                                omega = omega, raan = raan, 
                                                anomaly = anomaly,
                                                anomaly_type = anomaly_type,
                                                label = sat_name)
            ...
            sat3 = getattr(self.parent(), 'sat3', None)
            if sat3 == None:
                self.parent().sat3 = sat  
            else:
                self.parent().sat3.update_orbit(a = a, e = e, i = i, 
                                                omega = omega, raan = raan, 
                                                anomaly = anomaly,
                                                anomaly_type = anomaly_type,
                                                label = sat_name)
            ...
            sat4 = getattr(self.parent(), 'sat4', None)
            if sat4 == None:
                self.parent().sat4 = sat  
            else:
                self.parent().sat4.update_orbit(a = a, e = e, i = i, 
                                                omega = omega, raan = raan, 
                                                anomaly = anomaly,
                                                anomaly_type = anomaly_type,
                                                label = sat_name)
