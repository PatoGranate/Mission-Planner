...
class SatInfoWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    # Satellite information update function
    def update_info(self):
        try:
            # Try first setting duration, timestep and epoch
            ...
            # For each satellite, try displaying their information
            try: 
                ...
                self.info_display_i0.setText(str(int(round(np.rad2deg(sat0.i)))))
                self.info_display_omega0.setText(str(int(round(np.rad2deg(sat0.omega)))))
                self.info_display_raan0.setText(str(int(round(np.rad2deg(sat0.raan)))))
                self.info_display_anom0.setText(str(int(round(np.rad2deg(sat0.anomaly)))))
                self.info_display_anom_type0.setText(str(sat0.anomaly_type))
             
            # If any errors, don't display
        ...
        # If fail, still open window, don't update
        except AttributeError:
            pass